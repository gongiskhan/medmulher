...
jQuery(function($) {'use strict';
  ...
  // ======================================================
	...
	if (document.getElementById('btn_app_submit')) {
		document.getElementById('app_form').addEventListener('submit', app_form);
	}

	function app_form(event) {
		...
		let frm = document.getElementsByName('appointmentForm')[0];

		const body = {
			to: 'goncalo.p.gomes@gmail.com',
			from: 'appmobpt@gmail.com',
			subject: 'Marcação de consulta',
			content: `<p>Seguem abaixo os dados do utente que solicita o pedido de marcação de consulta:</p>
			<br>
			<p><b>Nome:</b> ${name}</p>
			<p><b>Contacto:</b> ${phone}</p>
			<p><b>E-mail:</b> ${email}</p>
			<p><b>Data de consulta:</b> ${date}</p>
			<p><b>Especialidade:</b> ${appointment}</p>
			<p><b>Mensagem:</b> ${message}</p>
			<br>
			<p>Pedido de marcação de consulta enviado através do website MedMulher.</p>`
		};

		fetch('https://us-central1-eatmatic-143319.cloudfunctions.net/httpGenericEmail', {
			method: 'POST',
			...
			.then((data) => {
				...
				alert('Pedido de marcação de consulta enviado com sucesso!');
				frm.reset();
			})
